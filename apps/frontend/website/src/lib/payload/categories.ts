...
export async function getPayloadCategories(): Promise<AdaptedCategory[]> {
  ...
  try {
    ...
    console.warn(
      `[payload] Failed to fetch categories from ${url}: ${err?.message || err}`,
    );
    return [];
  }
}
...
export async function getPayloadCategoryBySlug(
  slug: string,
): Promise<AdaptedCategory | null> {
  type ListResponse = {
    docs: PayloadCategory[];
  } & Record<string, unknown>;
  ...
  try {
    ...
    console.warn(
      `[payload] Failed to fetch category by slug from ${url}: ${err?.message || err}`,
    );
    return null;
  }
}
