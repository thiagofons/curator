...
function adapt(doc: any): AdaptedPost {
  ...
  const categories: string[] = catDocs
    .map((c) =>
      c && typeof c === "object" ? pickLocale<string>(c.title) : undefined,
    )
    .filter(Boolean) as string[];
  const categorySlugs: string[] | undefined = catDocs
    .map((c) =>
      c && typeof c === "object" && c.slug ? String(c.slug) : undefined,
    )
    .filter(Boolean) as string[];
...
export async function getPayloadPosts(options?: {
  ...
  try {
    ...
    console.warn(
      `[payload] Failed to fetch posts from ${url}. Error: ${err?.message || err}`,
    );
    return [];
  }
}
...
export async function getPayloadPostBySlug(
  ...
  try {
    ...
    console.warn(
      `[payload] Failed to fetch post by slug from ${url}. Error: ${err?.message || err}`,
    );
    return null;
  }
}
...
export async function getPayloadPostsByAuthorSlug(
  ...
  try {
    ...
    console.warn(
      `[payload] Failed to fetch posts by author from ${url}. Error: ${err?.message || err}`,
    );
    return [];
  }
}
...
export async function getPayloadPostsByCategorySlug(
  slug: string,
): Promise<AdaptedPost[]> {
  type ListResponse = {
    docs: any[];
  ...
  try {
    ...
    console.warn(
      `[payload] Failed to fetch posts by category from ${url}. Error: ${err?.message || err}`,
    );
    return [];
  }
}
