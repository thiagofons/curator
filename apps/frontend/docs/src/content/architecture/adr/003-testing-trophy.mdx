...
- Tooling standardized with our stack (TypeScript, Vite/Vitest, Prisma, Playwright).
...
#### 1. Static Analysis (The Base): TypeScript & BiomeJS
...
- Tools: `tsc` (TypeScript compiler) and `BiomeJS` (for linting/formatting).
...
flowchart TB
  ...
  SA["<b>Static Analysis</b><br/>TypeScript + BiomeJS"]
...
- Very fast feedback via TypeScript + Biome (catches trivial issues before running tests).
...
- Stack coherence with our ecosystem (Vite/Vitest, Prisma, Playwright) and strong DX.
...
- Description: Traditional lint/format stack (ESLint/Prettier).
- Reason for Rejection: Biome provides integrated lint/format and superior performance, simplifying maintenance and standardization.
